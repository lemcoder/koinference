 ...
 * killed, the model unloaded under you — is handed to the `onDeath` callback that
...
sealed class KoinferenceException(message: String, cause: Throwable? = null) :
    ...
    /** The model loaded, but cannot do what was asked — e.g. a connection to generate on an embedder. */
    class Unsupported(message: String) : KoinferenceException(message)

    /** [Koinference.openConnection] or [Koinference.unloadModel] was given a handle that is not loaded. */
    ...
    /** A call was made on a connection that has been closed. Call-scoped: thrown. [target] names the model. */
    class ConnectionClosed(target: String) :
        KoinferenceException("This connection has been closed: $target")
